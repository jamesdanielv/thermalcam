...
//LATEST UPDATE FEATURE:autotemp setting! 
...
MINTEMP=MINTEMP*0.8+(templow-2)*0.2;
MAXTEMP=MAXTEMP*0.8+(temphi+1)*0.2;
#endif
//*********
float tempr=0.0;
...
#endif
if (tempf<100){
tft.drawChar(128-22,128-13,48+charplace1a,0xFFFF,0xFFFF,2);//already in code if same color, transperant background!
tft.drawChar(128-11,128-13,48+charplace2a,0xFFFF,0xFFFF,2);//already in code if same color, transperant background!
}else{
//draw NA
tft.drawChar(128-11,128-13,65,0xFFFF,0xFFFF,2);//already in code if same color, transperant background!
tft.drawChar(128-22,128-13,78,0xFFFF,0xFFFF,2);//already in code if same color, transperant background!
}
...
#endif
if (tempf2>0){
tft.drawChar(0,128-13,48+charplace1b,0xFFFF,0xFFFF,2);//already in code if same color, transperant background!
tft.drawChar(11,128-13,48+charplace2b,0xFFFF,0xFFFF,2);//already in code if same color, transperant background!
}else{
//draw NA
tft.drawChar(11,128-13,65,0xFFFF,0xFFFF,2);//already in code if same color, transperant background!
tft.drawChar(0,128-13,78,0xFFFF,0xFFFF,2);//already in code if same color, transperant background!
...
//*******************************below lines are for making device into a thermal pointing device look in Adafruit_AMG88xx.cpp for mirroring of sensor data (in this version)
// look in Adafruit_AMG88xx.cpp file for a setting called #define AMG88xx_PIXEL_MIRROR //this mirrors image from left to right to right to left for sensor, such as when amg8833 device is pointing away from display. //this is better with included zip package that has all needed libraries in it for testing.
//most updated version of thermal_cam file can replace the one in zip file
//this is a work in process. 16x16 subsample seems to work ok, trying to verify 32x32 and add up to 512 by 512
//some changes may be needed to have includes location changes also spixx.h just uses libray in file change back to <spi.h>
//this is the only file where code changes are, so just replace old file with this one 
...
MINTEMP=MINTEMP*0.95+(templow-2)*0.05;
MAXTEMP=MAXTEMP*0.95+(temphi+1)*0.05;
#endif
//*********
float tempr=0.0;
...
timer=micros();//used for testing
   ...
   #if colorMode <2
    int colorIndex  =  Fasttempcachemap256(pixels[i+j*8]);
    //old way // int colorIndex = map(pixels[i+j*8], MINTEMP, MAXTEMP, 0, 255); //we resuse this sample 
    #else
    //old way //int colorIndex= (pixels[i+j*8]-MINTEMP)*1024/(MAXTEMP-MINTEMP);//
    int colorIndex= Fasttempcachemap1024(pixels[i+j*8]);
   #endif
...
if (i+j*8 !=(i+j*8)|slowupdate)    compressionnumber++;// we only count priority pixels, not ones with slower refresh
   ...
   //there is also spi optimizations to allow spi bursts during pixel writes if using included st77xx.h and st77xx.cpp files 
   #if colorMode <2
   colorIndex=colorIndex & 255;//faster constrain
   #else
   colorIndex=colorIndex &1023;
   #endif
   #if  subpixelcoloroptimized !=-1 //we test without lcd writes with -1
    tft.fillRect(displayPixelWidth *j, displayPixelHeight * i,displayPixelWidth, displayPixelHeight,(uint16_t)pgm_read_word_near(camColors+colorIndex));
    #endif
...
#if colorMode < 2
tempcolorx=tempcolorx&255;//subsample with real pixel and surounding pixels
#else
tempcolorx=tempcolorx&1023;//subsample with real pixel and surounding pixels
#endif
//#if interpolatemode  > 1
...
#if colorMode <2
int  tempcolor= map(pixels[(i+raster_y+interpolatesampledir2)+(j+raster_x)*8], MINTEMP, MAXTEMP, 0, 255);//we constrain color after subsampling
 #else
int  tempcolor= map(pixels[(i+raster_y+interpolatesampledir2)+(j+raster_x)*8], MINTEMP, MAXTEMP, 0, 1023);//we constrain color after subsampling
...
//#if  interpolatemode  > 1
//next line changes the average of the color between the main pixel and the sub pixels
tempcolorx32=(( tempcolorx32*(pixelSizeDividexx-raster_yxx)+ colorIndex32*raster_yxx)/pixelSizeDividexx+(tempcolorx32*(raster_xxx)+colorIndex32*(pixelSizeDividexx-raster_xxx))/pixelSizeDividexx)/2;//subsample with real pixel and surounding pixels



#if colorMode < 2
tempcolorx32=constrain(tempcolorx32,0,255);//subsample with real pixel and surounding pixels
#else
tempcolorx32=constrain(tempcolorx32,0,1023);//subsample with real pixel and surounding pixels
#endif
#if subpixelcoloroptimized == 3 //this is for writing 64 rectangles at a time!
subpixelbuffer2[color4buf2]=(uint16_t)pgm_read_word_near(camColors+tempcolorx32);
color4buf2++;//we count 0-3 and reset again, each pixel set and we draw 4 colors at a time
