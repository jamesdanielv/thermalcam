...
//#define SSPI_WRITE_PIXELS_MORE (c,l)  for(uint32_t i=0; i<(l); i+=2){ SSPI_WRITE(((uint8_t*)(c))[i+1]); SSPI_WRITE(((uint8_t*)(c))[i]); }
...
#define SPI_OBJECT  SPI
    ...
    #else
        #define HSPI_WRITE_PIXELS(c,l)   for(uint32_t i=0; i<((l)); i++){ SPI_WRITE16(((uint16_t*)(c))[i]); }
    #endif
#else
...
static inline uint8_t _avr_spi_read(void) {
    ...
    return r;
        ...
        #define HSPI_WRITE(b)            {SPDR = (b); }// {SPDR = (b); while(!(SPSR & _BV(SPIF)));}//while loop and check not needed on fast spi modes!
    ...
    #define HSPI_WRITE_PIXELS(c,l)   uint32_t i=0; while(i<(l)/2){ i+=2; HSPI_WRITE(((uint8_t*)(c))[i+1]); HSPI_WRITE(((uint8_t*)(c))[i]); }
    //#define SPI_WRITE_PIXELS_MORE (c,l)  if(_sclk < 0){HSPI_WRITE_PIXELS(c,l);}else{SSPI_WRITE_PIXELS(c,l);}
#endif
