...
#if lcd_mode == 0
#ifndef _ADAFRUIT_SPITFT_C
#define _ADAFRUIT_SPITFT_C
/***************************************************
...
void inline Adafruit_SPITFT::writePixels(uint16_t * colors, uint32_t len){
   SPI_WRITE_PIXELS((uint8_t*)colors , len *2 );//was *2 but we are doing 4 at a time
}

void Adafruit_SPITFT::writeColor(uint16_t color, uint32_t len){
#ifdef SPI_HAS_WRITE_PIXELS || SPI_HAS_WRITE_PIXELS_MORE
    if(_sclk >= 0){
